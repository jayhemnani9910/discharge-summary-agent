...
- **Measured result.** On a held-out split the reward model never trains on, mean edit burden
  drops from a **0.15 baseline to roughly 0.03** (about 80% lower) as training pairs accumulate,
  with safety retention at 100% throughout. The before/after curve and per-round table are in
  `outputs/learning/report.md` / `curve.svg`, regenerated by `learn`. The offline run uses a
  deterministic stand-in judge (so the curve is reproducible in CI with no key); `learn --live`
  uses a real LLM judge and `learn --patients <dir>` runs across real patient folders with no
  code change, which is the hidden-patient / production path.

Limitations of the Part 2 loop are listed below with the rest.

## Testing

`pytest -q` runs 69 offline tests (no API key). Part 1 coverage: the citation guardrail,
...
- **Part 2 offline judge.** The no-key `learn` run uses a deterministic stand-in judge so the
  curve reproduces in CI; the genuine in-context learning curve comes from `learn --live`. The
  real signal is real doctor edits, which drop into the same `EditSource` interface.
